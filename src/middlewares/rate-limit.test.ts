...
describe("rate limiting", () => {
  ...
  it("keeps separate budgets per keyPrefix", async () => {
    ...
    // Exhausting payments must not lock someone out of browsing.
...
describe("rate limits applied to real endpoints", () => {
  ...
  it("does not throttle the M-Pesa callback", async () => {
    // Safaricom retries, and dropping a callback loses track of real money.
    for (let i = 0; i < 20; i += 1) {
      const res = await realApp.request("/mpesa/callback", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ Body: { stkCallback: { CheckoutRequestID: "x", ResultCode: 1 } } }),
      });
